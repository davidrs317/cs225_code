...
class KDTree
{
  ...
  private:
    ...
    double find_Dist(const Point<Dim>& first, const Point<Dim>& sec) const; 

    /** Helper function to build a kD-Tree for the constructor */
    ...
    /** Helper function to perform necessary actions for findNearestNeighbor function */
    Point<Dim> nearestNeighbor(const Point<Dim>& query, int dim, KDTreeNode* curRoot) const;

    /** Helper function to partition the list for the quickselect algorithm */
    int partitioner(vector<Point<Dim>>& points, int left, int right, int PivotIndex, int dim);

    /** Helper function to delete a tree */
    void _clear(KDTreeNode*& curRoot);

    /** Helper function to copy a tree */
    void _copy(const KDTreeNode* curRoot, KDTreeNode*& newbie);
